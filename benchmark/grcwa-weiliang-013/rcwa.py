...
class obj:
    def __init__(
        self, nG, L1, L2, freq, theta, phi, verbose=1, fmm_method=None, pol_sigma=3.0, pol_niter=20
    ):
        ...
        for i in range(self.Layer_N):
            ...
            if self.fmm_method == "pol" and not is_anisotropic:
                epinv, ep2 = Epsilon_fft_pol(
                    dN, ep_grid, self.G, pol_sigma=self.pol_sigma, pol_niter=self.pol_niter
                )
            else:
    ...
    def RT_Solve(self, normalize=0, byorder=0):
        ...
        """
        aN, b0 = SolveExterior(
            self.a0, self.bN, self.q_list, self.phi_list, self.kp_list, self.thickness_list
        )
    ...
    def Solve_FieldFourier(self, which_layer, z_offset):
        ...
        for zoff in zl:
            ai, bi = TranslateAmplitudes(
                self.q_list[which_layer], self.thickness_list[which_layer], zoff, ai0, bi0
            )
...
def GetSMatrix(indi, indj, q_list, phi_list, kp_list, thickness_list):
    ...
    for li in range(indi, indj):
        ## next layer
        lp1 = li + 1

        ## Q = inv(phi_l) * phi_lp1
        Q = bd.dot(bd.inv(phi_list[li]), phi_list[lp1])
        ## P = ql*inv(kp_l*phi_l) * kp_lp1*phi_lp1*q_lp1^-1
        P1 = bd.dot(bd.diag(q_list[li]), bd.inv(bd.dot(kp_list[li], phi_list[li])))
        ...
        # phase
        d1 = bd.diag(bd.exp(1j * q_list[li] * thickness_list[li]))
