...
public class MooseScript
{
    ...
    // Cost guard for the smallest-first/non-2D side.  After a non-2D solve
    // exceeds this many seconds, its still-unclaimed higher orders are skipped.
    // The dedicated descending 2D side intentionally ignores this guard so it
    // can keep walking from the largest unfinished 2D jobs downward.
    // 0 = no limit.
    ...
    // single-threaded -- the eigenproblem is not parallelized -- so the sweep
    // runs independent structures on worker threads, each with its own Rcwa
    // instance.
    //   1 = one solve at a time.
    //   N = up to N solves at once.
    //   0 = use Environment.ProcessorCount.
    // Per-solve timings get noisier because concurrent solves share memory
    // bandwidth, so a timing/calibration run still wants 1.
    // MEMORY SCALES WITH THIS.  Each concurrent solve holds its own matrices.
    static int    PARALLEL_TASKS  = 30;

    // Two-ended production scheduler.  Up to this many workers always claim
    // the largest unfinished 2D job first.  The remaining workers claim the
    // smallest unfinished job first.  Once no 2D jobs remain, the reserved
    // workers automatically join the smallest-first side.
    static int    LARGE_2D_WORKERS = 8;
    ...
    // Main sweep uses one shared set of unfinished jobs.  High workers claim
    // from the 2D/high-nG end; normal workers claim from the low-nG end.
    // Claimed is changed only while holding sLock, so a job cannot be run twice.
    class WorkItem
    {
        public BenchCase Case;
        public int M;
        public bool Claimed;
        public WorkItem(BenchCase c, int m)
        {
            Case = c; M = m; Claimed = false;
        }
    }

    static List<WorkItem> sTwoEndedWork = new List<WorkItem>();
    static Dictionary<string, int> sBudgetCutoffM = new Dictionary<string, int>();
    ...
    // True when the non-2D cost guard has ruled out this higher-order point.
    // The 2D descending lane intentionally ignores MAX_SECONDS_PER_SOLVE:
    // otherwise a smaller 2D solve completed by the low side could cancel the
    // very large 2D tail that this scheduler is specifically meant to run.
    static bool BudgetSkips(WorkItem w)
    {
        if (w.Case.Dim == 2) return false;
        int cutoff;
        if (!sBudgetCutoffM.TryGetValue(w.Case.Name, out cutoff)) return false;
        return w.M > cutoff;
    }

    static void MarkBudgetSkipsLocked()
    {
        for (int i = 0; i < sTwoEndedWork.Count; i++)
        {
            WorkItem w = sTwoEndedWork[i];
            if (w.Claimed || !BudgetSkips(w)) continue;
            w.Claimed = true;
            Io.output("  skip (budget) " + Pad(w.Case.Name, 26)
                      + " m=" + w.M.ToString(INV));
        }
    }

    static WorkItem ClaimSmallestLocked()
    {
        MarkBudgetSkipsLocked();
        int best = -1;
        long bestNg = long.MaxValue;
        for (int i = 0; i < sTwoEndedWork.Count; i++)
        {
            WorkItem w = sTwoEndedWork[i];
            if (w.Claimed) continue;
            long ng = NgOf(w.Case, w.M);
            if (best < 0 || ng < bestNg)
            {
                best = i; bestNg = ng;
            }
        ...
        sTwoEndedWork[best].Claimed = true;
        return sTwoEndedWork[best];
    }

    static WorkItem ClaimLargest2DLocked()
    {
        MarkBudgetSkipsLocked();
        int best = -1;
        long bestNg = long.MinValue;
        for (int i = 0; i < sTwoEndedWork.Count; i++)
        {
            WorkItem w = sTwoEndedWork[i];
            if (w.Claimed || w.Case.Dim != 2) continue;
            long ng = NgOf(w.Case, w.M);
            if (best < 0 || ng > bestNg)
            {
                best = i; bestNg = ng;
        ...
        // No 2D work left: this reserved worker joins the low side.
        return ClaimSmallestLocked();
    }

    static void ApplyBudgetAfterRun(BenchCase c, BenchResult r)
    {
        if (c.Dim == 2 || MAX_SECONDS_PER_SOLVE <= 0.0) return;
        if (r.TSolve <= MAX_SECONDS_PER_SOLVE) return;
        lock (sLock)
        {
            int old;
            if (!sBudgetCutoffM.TryGetValue(c.Name, out old) || r.M < old)
                sBudgetCutoffM[c.Name] = r.M;
            Io.output("  -> " + c.Name + ": solve took "
                      + F(r.TSolve, 1) + "s > budget "
                      + F(MAX_SECONDS_PER_SOLVE, 0)
                      + "s, skipping its still-unclaimed higher orders");
        }
    }

    static void TwoEndedWorker(bool largest2D)
    {
        while (true)
        {
            WorkItem w;
            lock (sLock)
            {
                w = largest2D ? ClaimLargest2DLocked() : ClaimSmallestLocked();
            }
            if (w == null) return;
            ...
            BenchResult r;
            if (PARALLEL_NG_LIMIT > 0 && ng > PARALLEL_NG_LIMIT)
                lock (sBigLock) { r = RunOne(w.Case, w.M); }
            else
                r = RunOne(w.Case, w.M);

            Record(w.Case, r);
            ApplyBudgetAfterRun(w.Case, r);
        }
    }

    static void High2DWorker() { TwoEndedWorker(true); }
    static void SmallFirstWorker() { TwoEndedWorker(false); }

    static void RunTwoEndedQueue(List<WorkItem> jobs)
    {
        sTwoEndedWork = jobs;
        sBudgetCutoffM = new Dictionary<string, int>();
        sBatch = new Dictionary<string, BenchResult>();

        int workers = Math.Min(WorkerCount(), jobs.Count);
        if (workers <= 0) return;

        int high = LARGE_2D_WORKERS;
        if (high < 0) high = 0;
        if (high > workers) high = workers;
        int low = workers - high;

        Io.output(" scheduler    : " + high.ToString(INV)
                  + " workers largest-2D -> down, "
                  + low.ToString(INV) + " workers smallest -> up");

        if (workers == 1)
        {
            if (high == 1) High2DWorker();
            else SmallFirstWorker();
            return;
        }
        ...
        {
            pool[at] = new Thread(new ThreadStart(High2DWorker));
            pool[at].IsBackground = false;
            pool[at].Start();
            at++;
        }
        for (int i = 0; i < low; i++)
        {
            pool[at] = new Thread(new ThreadStart(SmallFirstWorker));
            pool[at].IsBackground = false;
            pool[at].Start();
            at++;
        }
    ...
    static void Main()
    {
        ...
        // Build the complete unfinished sweep once.  Resume rows are skipped
        // here, before any worker starts, so restarting the script preserves
        // the old resume behaviour without duplicating a solve.
        List<WorkItem> jobs = new List<WorkItem>();
        for (int i = 0; i < cases.Count; i++)
        {
            BenchCase c = cases[i];
